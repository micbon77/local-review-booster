...
interface Subscriber {
  ...
  user_id: string; // Ensure we map user_id if needed for delete, or just use id? The interface uses id, but the DELETE API likely expects userId.
}
// Checking prior file view: DELETE expects { userId }.
// The subscriber object from DB typically has `user_id` column.
// I need to make sure the interface reflects that. Let's assume the API returns DB columns.

export default function EmailMarketing({ userId }: { userId: string }) {
  // ... existing state ...

  const handleDelete = async (targetUserId: string) => {
    if (!confirm("Sei sicuro di voler rimuovere questo iscritto?")) return;
    ...
    try {
      ...
      if (response.ok) {
        // Remove locally
        setSubscribers((prev) => prev.filter((s) => s.user_id !== targetUserId));
      } else {
